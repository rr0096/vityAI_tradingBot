...
try:
    ...
    METRICS_AVAILABLE = False
                ...
                logger.warning(f"[{self.agent_type}] Failed to record metrics: {e}")
...
class EnhancedBaseLLMAgent(Agent, ABC):
    ...
    def _setup_clients(self):
        ...
        try:
            ...
            if self._supports_tools and self._raw_client:
                try:
                ...
                if self._supports_tools:
                     logger.warning(f"[{self.agent_type}] Tools supported by config, but raw client failed. Instructor client not set up.")
...
Now, provide ONLY the JSON object based on the analysis:"""
    ...
    def _create_safe_default(self, response_model: Type[T], reason: str) -> Optional[T]:
        ...
        try:
            for field_name, field_info in response_model.model_fields.items():
                ...
                if field_type == str: defaults[field_name] = f"Default value due to error: {reason[:50]}"
                ...
                elif 'signal' in field_name.lower() and 'Literal' in str(field_info.annotation):
                    ...
                    if non_none_literal is not None:
                        ...
                        logger.error(f"Could not determine default for Literal field {field_name}")
                elif 'confidence' in field_name.lower() and (field_type == float or field_type == Optional[float]):
    ...
    def _record_agent_metrics(self, execution_time_ms: float, success: bool, 
                            error_message: Optional[str] = None, 
                            confidence_score: Optional[float] = None,
                            signal: Optional[str] = None):
        """Registra métricas de ejecución del agente"""
        if METRICS_AVAILABLE:
            try:
                ...
                logger.debug(f"Error recording agent metrics: {e}")
