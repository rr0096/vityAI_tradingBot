...
class EnhancedChartGenerator:
    ...
    def __init__(self, save_charts_to_disk: bool = False, charts_dir_str: str = "logs/charts"):
        ...
        self.style_rc_params = {
            ...
            'axes.facecolor': self.axes_bg_color,
            'xtick.color': self.text_color, 
            'ytick.color': self.text_color,
            'axes.labelcolor': self.text_color, 
            'axes.titlecolor': self.text_color,
            'text.color': self.text_color,
            'figure.dpi': 50,  # DPI bajo para imágenes más pequeñas
            'savefig.dpi': 50,  # DPI bajo para archivos pequeños
            'savefig.bbox': 'tight',
    ...
    def _prepare_and_validate_dataframe(
        ...
        try:
            cl = list(close_prices); hi = list(high_prices); lo = list(low_prices); vo = list(volumes)
            
            min_len_data = min(len(cl), len(hi), len(lo), len(vo))
            # Limit data to prevent huge images - maximum 50 data points for better visualization
            max_data_points = 50
            limited_lookback = min(lookback_periods, max_data_points)
            ...
            cl = cl[-actual_lookback:]; hi = hi[-actual_lookback:]; lo = lo[-actual_lookback:]; vo = vo[-actual_lookback:]
    ...
    def _create_chart(
        ...
        try:
            ...
            fig, axes = mpf.plot(
                ...
                figsize=(6, 4), returnfig=True, panel_ratios=panel_ratios_config,
                figscale=0.4, tight_layout=True, show_nontrading=False,
    ...
    def _fig_to_base64(self, fig: Figure) -> str:
        ...
        try:
            buf = BytesIO()
            fig.savefig(buf, format='png', dpi=50,  # Lower DPI for smaller images
                        facecolor=fig.get_facecolor(), bbox_inches='tight', pad_inches=0.1)
            plt.close(fig)
            
            # Get the image data and check size
            image_data = buf.getvalue()
            
            # Check if image needs resizing (only if extremely large)
            ...
            try:
                with Image.open(BytesIO(image_data)) as img:
                    ...
                    # Define reasonable limits - only resize if EXTREMELY large
                    max_width = 2000   # Higher limit to avoid unnecessary resizing
                    max_height = 1500  # Higher limit to avoid unnecessary resizing
                    min_width = 400    # Ensure minimum width for visibility
                    min_height = 300   # Ensure minimum height for visibility
                    
                    # Only resize if image is extremely large
                    if width > max_width or height > max_height:
                        # Calculate new size maintaining aspect ratio
                        ratio = min(max_width / width, max_height / height)
                        new_width = max(min_width, int(width * ratio))
                        new_height = max(min_height, int(height * ratio))
                        
                        # Double check that we're not creating a collapsed image
                        if new_width < min_width or new_height < min_height:
                            logger.warning(f"Calculated resize would create too small image ({new_width}x{new_height}). Keeping original.")
                        else:
                            # Resize the image
                            ...
                            resized_buf = BytesIO()
                            resized_img.save(resized_buf, format='PNG')
                            image_data = resized_buf.getvalue()
                            resized_buf.close()
                            
                            logger.info(f"Resized image from {width}x{height} to {new_width}x{new_height}")
                    
                    # Log final image size for verification
                    final_img = Image.open(BytesIO(image_data))
                    final_width, final_height = final_img.size
                    logger.info(f"Final image size: {final_width}x{final_height}")
                    final_img.close()
                    
            except Exception as resize_error:
                logger.warning(f"Could not check/resize image: {resize_error}")
    ...
    def _generate_error_chart(self, error_message: str) -> str:
        fig = None
        try:
            fig, ax = plt.subplots(figsize=(8, 4.5), facecolor=self.bg_color)
